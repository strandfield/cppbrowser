...
<template>
  ...
  <div v-if="symbol">
    ...
    <template v-if="isClass">
      <p v-if="symbol.baseClasses && symbol.baseClasses.length > 0">
        ...
        <template v-for="(base, index) in symbol.baseClasses" :key="index">
          <RouterLink
            :to="{ name: 'symbol', params: { projectName: projectName, projectRevision: projectRevision, symbolId: base.baseClassID } }">{{ base.name }}</RouterLink>{{ index == symbol.baseClasses.length - 1 ? "." : "," }}
        </template>
      </p>
      <p v-if="symbol.derivedClasses && symbol.derivedClasses.length > 0">
        ...
        <template v-for="(derived, index) in symbol.derivedClasses" :key="index">
          <RouterLink
            :to="{ name: 'symbol', params: { projectName: projectName, projectRevision: projectRevision, symbolId: derived.derivedClassID } }">{{ derived.name }}</RouterLink>{{ index == symbol.derivedClasses.length - 1 ? "." : "," }}
        </template>
      </p>
    </template>
      ...
      <table>
        <tbody>
          <tr v-for="child in symbol.namespaces" :key="child.id">
            <td>
              <RouterLink
                :to="{ name: 'symbol', params: { projectName: projectName, projectRevision: projectRevision, symbolId: child.id } }">{{ child.name }}</RouterLink>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table>
        <tbody>
          <tr v-for="child in symbol.functions" :key="child.id">
            <td>
              <RouterLink
                :to="{ name: 'symbol', params: { projectName: projectName, projectRevision: projectRevision, symbolId: child.id } }">{{ child.displayName ? child.displayName : child.name }}</RouterLink>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <template v-if="(symbol.constructors && symbol.constructors.length > 0) || (symbol.methods && symbol.methods.length > 0) ">
      <h3>Functions</h3>
      <table>
        <tbody>
    ...
    <template v-if="symbol.references && symbol.references.length > 0">
      ...
      <p v-for="refsInFile in symbol.references" :key="refsInFile.filePath">
        ...
        <template v-for="(refinfo, index) in refsInFile.references" :key="index">
          <RouterLink
            :to="{ name: 'file', params: { projectName: projectName, projectRevision: projectRevision, pathParts: getPathParts(refsInFile.filePath) }, hash: getHashForRef(refinfo)}">{{ refinfo.line }}</RouterLink>{{ index == refsInFile.references.length - 1 ? "." : ", " }}
        </template>
      </p>
    </template>
  </div>
